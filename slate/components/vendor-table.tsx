...
import { fetchVendors, deleteVendor, bulkDeleteVendors, type Vendor } from "@/lib/vendor-api"
...
export function VendorTable() {
  ...
  async function handleDelete(vendor: Vendor) {
    ...
    try {
      ...
      toast.success(`${vendor.kode} - ${vendor.nama} deleted`)
      loadData()
    } catch {
      toast.error(`Failed to delete ${vendor.kode}`)
    } finally {
      setDeleting(false)
  ...
  async function handleBulkDelete() {
    ...
    setDeleting(true)
    try {
      await bulkDeleteVendors(items.map((i) => i.id))
      toast.success(
        items
          .map((i) => `${i.kode} - ${i.nama}`)
          .join(", ") + " deleted"
            ...
            {loading ? (
              ...
              table.getRowModel().rows.map((row) => (
                <TableRow
                  ...
                  onClick={() => setDetailVendor(row.original)}
                >
