...
import { fetchJenisPajak, deleteJenisPajak, bulkDeleteJenisPajak, type JenisPajak } from "@/lib/jenis-pajak-api"
import { AddJenisPajakSheet } from "@/components/add-jenis-pajak-sheet"

export function JenisPajakTable() {
  const [data, setData] = useState<JenisPajak[]>([])
  ...
  const [editItem, setEditItem] = useState<JenisPajak | null>(null)
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false)
  const [bulkDeleteDialogOpen, setBulkDeleteDialogOpen] = useState(false)
  const [deletingItem, setDeletingItem] = useState<JenisPajak | null>(null)
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetchJenisPajak({
  ...
  async function handleDelete(item: JenisPajak) {
    setDeleting(true)
    try {
      await deleteJenisPajak(item.id)
      toast.success(`${item.nama} deleted`)
      loadData()
    } catch {
      toast.error(`Failed to delete ${item.nama}`)
    } finally {
      setDeleting(false)
  ...
  async function handleBulkDelete() {
    ...
    setDeleting(true)
    try {
      await bulkDeleteJenisPajak(items.map((i) => i.id))
      toast.success(items.map((i) => i.nama).join(", ") + " deleted")
      setRowSelection({})
      loadData()
    } catch {
  ...
  const columns: ColumnDef<JenisPajak>[] = useMemo(() => [
    {
      id: "select",
      header: ({ table }) => (
    ...
    { accessorKey: "nama", header: "Nama Pajak" },
    {
    ...
    <div className="flex w-full flex-col gap-4">
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent size="sm">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {deletingItem ? `${deletingItem.nama}. ` : ""}
      ...
      <AlertDialog open={bulkDeleteDialogOpen} onOpenChange={setBulkDeleteDialogOpen}>
        <AlertDialogContent size="sm">
          <AlertDialogHeader>
            <AlertDialogMedia><Trash2Icon className="text-destructive" /></AlertDialogMedia>
            <AlertDialogTitle>Delete {selectedCount} tax type(s)?</AlertDialogTitle>
            <AlertDialogDescription>
              {table.getSelectedRowModel().rows.map((r) => r.original.nama).join(", ")}<br />
              This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AddJenisPajakSheet open={sheetOpen} onOpenChange={(open) => { if (!open) setEditItem(null); setSheetOpen(open) }} onSuccess={loadData} editItem={editItem} />
    </div>
  )
}
