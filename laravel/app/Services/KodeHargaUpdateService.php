...
class KodeHargaUpdateService
{
    public function generate(?\Carbon\Carbon $tanggal = null): string
    {
        ...
        return DB::transaction(function () use ($tanggal) {
            $setting = Setting::where('group', 'harga_update')
                ->lockForUpdate()
                ->firstOrFail();
            ...
            for ($i = 0; $i < $maxAttempts; $i++) {
                ...
                $kode = str_replace(
                    ...
                    $data['format_kode'] ?? 'HU-{Y}-{M}-{seq}'
                );

                $exists = HargaUpdate::where('kode', $kode)->exists();
                if (!$exists) {
            ...
            throw new \RuntimeException("Unable to generate unique HU code after {$maxAttempts} attempts");
        });
    }
