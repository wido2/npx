...
class KodePBService
{
    public function generate(?\Carbon\Carbon $tanggal = null): string
    {
        ...
        return DB::transaction(function () use ($tanggal) {
            $setting = Setting::where('group', 'pengambilan_barang')
                ->lockForUpdate()
                ->firstOrFail();
            ...
            for ($i = 0; $i < $maxAttempts; $i++) {
                ...
                $kode = str_replace(
                    ...
                    $data['format_kode'] ?? 'PB-{Y}-{M}-{seq}'
                );

                $exists = PengambilanBarang::where('kode', $kode)->exists();
                if (!$exists) {
            ...
            throw new \RuntimeException("Unable to generate unique PB code after {$maxAttempts} attempts");
        });
    }
